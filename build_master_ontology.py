...
csv_data = {k: [] for k in ['Process', 'Risk', 'Strategy'] + list(dict_keywords.keys())}
rels = []

# ID 접두어 매핑
prefixes = {
    'Process': 'Proc_', 'Risk': 'Risk_', 'Strategy': 'Strat_',
    ...
    def get_id(cat, name, extra_props=None):
        if name not in nodes[cat]:
            ...
            if cat == 'Process': row_data['name'] = name
                ...
                row_data.update({'description': name, 'source_project': proj})
            elif cat == 'Strategy': 
                row_data.update({'action': name, 'source_project': proj})
            else:
